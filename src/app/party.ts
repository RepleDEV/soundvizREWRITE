...
function get_color(colors: Colors, pos: number): Color {
    ...
    if (positions.includes(limit_pos)) {
        ...
        let diff = (limit_pos - pos_low - (100 - pos_high)) / (pos_high - pos_low) * 100;

        if (diff < 0)diff += 100;

        const final_color: Color = [0,0,0];
        for (let j = 0;j < 3;j++) {
            final_color[j] = (color_high[j] * diff / 100) + (color_low[j] * (100 - diff) / 100);
        }
...
function get_volume(analyser: AnalyserNode, volume_ceil: number, multiplier?: number): number {
